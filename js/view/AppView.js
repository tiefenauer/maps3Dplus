...
define([
	...
	function($, _, Backbone, GoogleMapsView, ProfileView, SettingsPopupView, ProfileModel, ProfileDao, map_stats, profile_stats, progress_popup, settings_popup, info_popup){
		...
		var AppView = Backbone.View.extend({
			...
			*/
			events: {
				...
				'click #loadButton' : 'onLoadButtonClick',
			...
			*/
			initialize: function(){
				...
				var loadButton = $('<button></button>').attr({id: "loadButton", type: "file", class: "btn btn-default"}).append($('<span></span>').attr({class: 'glyphicon glyphicon-open'}));
				var saveButton = $('<button></button>').attr({id: "saveButton", type: "button", class: "btn btn-default"}).append($('<span></span>').attr({class: 'glyphicon glyphicon-save'}));
				...
				this.listenTo(this.profileDao, 'io:model:loaded', this.onModelLoaded);
				this.mapView.render();
			},
			...
			*/
			onSubmitButtonClick: function(event)
			{
				...
				this.profileModel = new ProfileModel({profilePoints: mapView.coordinates});
			...
			*/
			...
			onLoadButtonClick: function(event)
			{
				var self = this;
				...
				fileSelector.change(function(evt){
					...
					if (file)
						self.profileDao.load(file);			
				});
				fileSelector.click();				
			},
			onModelLoaded: function(geometry)
			{
				console.log('Model loaded from file. Processing ...');
				this.profileView.drawGeometry(geometry);
			},
			onSaveButtonClick: function(event)
			{
				this.profileDao.save('dummyFile', this.profileView.getObjects());
			},			
			/**
